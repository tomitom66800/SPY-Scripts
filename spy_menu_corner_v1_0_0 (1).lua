...
local MENU_NAME = "SPY MENU - THE CORNER"
...
local SpyMenu = {
    ...
    ProximityTracking = {}, -- Joueurs proches de la target et leur temps
    RoomData = {}, -- Données des rooms
...
local function GetPlayerAttributes(player)
    ...
    -- Liste des attributs The Corner
    local knownAttrs = {
        "DonationGave",
        "DonationReceived",
        "Reputation",
        ...
        "NumberOfSentComments",
        "PlayerTrackPermission",
        "ShowCommentBubble",
        "HideCountry",
        "HideStreak",
        "Tags",
...
-- Calculer le K/D (Robux Donnés / Reçus)
...
local function GetPlayerInfo(player)
    local info = {
        ...
        IsAbove18 = "N/A",
        GroupRank = "N/A",
        NumberOfReceivedComments = "N/A",
        NumberOfSentComments = "N/A",
        Tags = "N/A",
        PrivateRoom = "N/A"
    }
    
    -- Attributs The Corner
    ...
    if attrs.Reputation then
        info.Reputation = tostring(attrs.Reputation)
    end
    ...
    -- GroupRank
    if attrs.GroupRank then
        info.GroupRank = tostring(attrs.GroupRank)
    end
    
    -- Comments
    if attrs.NumberOfReceivedComments then
        info.NumberOfReceivedComments = tostring(attrs.NumberOfReceivedComments)
    end
    if attrs.NumberOfSentComments then
        info.NumberOfSentComments = tostring(attrs.NumberOfSentComments)
    end
    
    -- Tags
    if attrs.Tags then
        info.Tags = tostring(attrs.Tags)
    end
    
    -- PrivateRoom
    if attrs.PrivateRoom ~= nil then
        info.PrivateRoom = attrs.PrivateRoom and "🔒 Dans une room" or "🌍 Publique"
    end
...
-- Fonction pour détecter les joueurs proches (à portée de voix - environ 80 studs)
local function GetNearbyPlayers(targetPlayer, maxDistance)
...
-- Fonction pour détecter les joueurs dans la même room privée
local function GetPlayersInSameRoom(targetPlayer)
    local roomPlayers = {}
    
    -- Vérifier si la target est dans une private room
    local targetInRoom = targetPlayer:GetAttribute("PrivateRoom")
    if not targetInRoom then
        return roomPlayers -- Pas dans une room, retourner vide
    end
    ...
    local targetPos = targetPlayer.Character.HumanoidRootPart.Position
    local ROOM_DISTANCE = 100 -- Dans les rooms privées, distance max pour être considéré ensemble
    
    -- Chercher tous les autres joueurs qui sont aussi dans une private room
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= targetPlayer then
            local playerInRoom = player:GetAttribute("PrivateRoom")
            
            -- Si le joueur est aussi dans une private room
            if playerInRoom and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                local playerPos = player.Character.HumanoidRootPart.Position
                local distance = (targetPos - playerPos).Magnitude
                
                -- Si proche dans une room privée, ils sont dans la même room
                if distance <= ROOM_DISTANCE then
...
-- Fonction pour obtenir toutes les rooms privées actives du serveur
local function GetAllRooms()
    local rooms = {}
    local processedPlayers = {}
    
    for _, player in pairs(Players:GetPlayers()) do
        -- Vérifier si le joueur est dans une private room et pas encore traité
        local inPrivateRoom = player:GetAttribute("PrivateRoom")
        
        if inPrivateRoom and not processedPlayers[player.UserId] then
            local roommates = GetPlayersInSameRoom(player)
            
            -- Si le joueur est dans une room (même seul)
            local room = {
                Host = player,
                Players = {player},
                IsPrivateRoom = true
            ...
            -- Ajouter les autres joueurs de la room
            for _, data in ipairs(roommates) do
                table.insert(room.Players, data.Player)
                processedPlayers[data.Player.UserId] = true
            end
...
local function OpenScanWindow(player)
    ...
    AddSection("🏙️ THE CORNER DATA", order)
    order = order + 1
    AddInfoLine("Reputation", info.Reputation, order)
    order = order + 1
    ...
    order = order + 1
    AddInfoLine("Group Rank", info.GroupRank, order)
    order = order + 1
    AddInfoLine("💬 Commentaires reçus", info.NumberOfReceivedComments, order)
    order = order + 1
    AddInfoLine("💬 Commentaires envoyés", info.NumberOfSentComments, order)
    order = order + 1
    AddInfoLine("🏷️ Tags", info.Tags, order)
    order = order + 1
    AddInfoLine("Room", info.PrivateRoom, order)
    order = order + 1
...
local function OpenSpyWindow(player)
    ...
    spyDirectTitle.Text = "🔍 SPY DIRECT - PROXIMITÉ & ROOM"
    ...
    proximityLabel.Size = UDim2.new(1, -20, 0, 20)
    proximityLabel.Position = UDim2.new(0, 10, 0, 35)
    proximityLabel.BackgroundTransparency = 1
    proximityLabel.Text = "👥 Joueurs à portée de voix (< 80 studs)"
    proximityLabel.TextColor3 = Colors.Warning
    proximityLabel.TextSize = 12
    proximityLabel.Font = Enum.Font.GothamBold
    proximityLabel.TextXAlignment = Enum.TextXAlignment.Left
    proximityLabel.Parent = spyDirectFrame
    
    local proximityScroll = Instance.new("ScrollingFrame")
    proximityScroll.Size = UDim2.new(1, -20, 0, 130)
    proximityScroll.Position = UDim2.new(0, 10, 0, 60)
    proximityScroll.BackgroundColor3 = Colors.Tertiary
    proximityScroll.BorderSizePixel = 0
    proximityScroll.ScrollBarThickness = 4
    proximityScroll.ScrollBarImageColor3 = Colors.Accent
    proximityScroll.Parent = spyDirectFrame
    
    local proximityCorner = Instance.new("UICorner")
    proximityCorner.CornerRadius = UDim.new(0, 6)
    proximityCorner.Parent = proximityScroll
    
    local proximityLayout = Instance.new("UIListLayout")
    proximityLayout.Padding = UDim.new(0, 3)
    proximityLayout.Parent = proximityScroll
    
    -- Section Room
    local roomLabel = Instance.new("TextLabel")
    roomLabel.Size = UDim2.new(1, -20, 0, 20)
    roomLabel.Position = UDim2.new(0, 10, 0, 200)
    roomLabel.BackgroundTransparency = 1
    roomLabel.Text = "🔒 Private Room - Membres"
    roomLabel.TextColor3 = Colors.Success
    roomLabel.TextSize = 12
    roomLabel.Font = Enum.Font.GothamBold
    roomLabel.TextXAlignment = Enum.TextXAlignment.Left
    roomLabel.Parent = spyDirectFrame
    
    local roomScroll = Instance.new("ScrollingFrame")
    roomScroll.Size = UDim2.new(1, -20, 0, 130)
    roomScroll.Position = UDim2.new(0, 10, 0, 225)
    roomScroll.BackgroundColor3 = Colors.Tertiary
    roomScroll.BorderSizePixel = 0
    roomScroll.ScrollBarThickness = 4
    roomScroll.ScrollBarImageColor3 = Colors.Accent
    roomScroll.Parent = spyDirectFrame
    
    local roomCorner = Instance.new("UICorner")
    roomCorner.CornerRadius = UDim.new(0, 6)
    roomCorner.Parent = roomScroll
    
    local roomLayout = Instance.new("UIListLayout")
    roomLayout.Padding = UDim.new(0, 3)
    ...
    updateConnection = RunService.Heartbeat:Connect(function()
        ...
        if player and player.Parent then
            ...
            -- Micro
            local micStatus = info.VoiceChatEnabled == "true" and "🟢 Activé" or "🔴 Désactivé"
            micLabel.Text = "🎤 Voice Chat: " .. micStatus
            ...
            -- Vider la liste proximité
            for _, child in pairs(proximityScroll:GetChildren()) do
            ...
            if #nearbyPlayers > 0 then
                for i, data in ipairs(nearbyPlayers) do
                    ...
                    -- Calculer le temps depuis qu'il est proche
                    local currentTime = os.time()
                    local trackingKey = player.UserId .. "_" .. targetPlayer.UserId
                    
                    if not SpyMenu.ProximityTracking[trackingKey] then
                        SpyMenu.ProximityTracking[trackingKey] = currentTime
                    end
                    
                    local timeNearby = currentTime - SpyMenu.ProximityTracking[trackingKey]
                    local minutes = math.floor(timeNearby / 60)
                    local seconds = timeNearby % 60
                    ...
                    playerCard.Parent = proximityScroll
                    ...
                    infoLabel.Position = UDim2.new(0, 10, 0, 19)
                    infoLabel.BackgroundTransparency = 1
                    infoLabel.Text = string.format("📏 %d studs | ⏱️ %s", data.Distance, timeText)
                ...
                proximityScroll.CanvasSize = UDim2.new(0, 0, 0, proximityLayout.AbsoluteContentSize.Y + 10)
            else
                local emptyLabel = Instance.new("TextLabel")
                emptyLabel.Size = UDim2.new(1, -10, 0, 18)
                emptyLabel.BackgroundTransparency = 1
                emptyLabel.Text = "Aucun joueur à proximité"
                emptyLabel.TextColor3 = Colors.TextSecondary
                emptyLabel.TextSize = 11
                emptyLabel.Font = Enum.Font.Gotham
                emptyLabel.TextXAlignment = Enum.TextXAlignment.Center
                emptyLabel.Parent = proximityScroll
                
                -- Nettoyer le tracking pour ce joueur
                for key, _ in pairs(SpyMenu.ProximityTracking) do
                    if key:find("^" .. player.UserId .. "_") then
                        SpyMenu.ProximityTracking[key] = nil
                    end
                end
            end
            
            -- SPY DIRECT: Mise à jour de la room
            local roomPlayers = GetPlayersInSameRoom(player)
            
            -- Vider la liste room
            for _, child in pairs(roomScroll:GetChildren()) do
                if child:IsA("Frame") then
                    child:Destroy()
                end
            end
            ...
            if #roomPlayers > 0 then
                for i, data in ipairs(roomPlayers) do
                    local targetPlayer = data.Player
                    ...
                    distLabel.Size = UDim2.new(0.4, -10, 1, 0)
                    distLabel.Position = UDim2.new(0.6, 0, 0, 0)
                    distLabel.BackgroundTransparency = 1
                    distLabel.Text = "📏 " .. data.Distance .. " studs"
                    distLabel.TextColor3 = Colors.Success
                    distLabel.TextSize = 10
                    distLabel.Font = Enum.Font.GothamBold
                    distLabel.TextXAlignment = Enum.TextXAlignment.Right
                    distLabel.Parent = playerCard
                end
                roomScroll.CanvasSize = UDim2.new(0, 0, 0, roomLayout.AbsoluteContentSize.Y + 10)
            else
                local emptyLabel = Instance.new("TextLabel")
                emptyLabel.Size = UDim2.new(1, -10, 0, 18)
                emptyLabel.BackgroundTransparency = 1
                emptyLabel.Text = "Pas dans une private room"
                emptyLabel.TextColor3 = Colors.TextSecondary
                emptyLabel.TextSize = 11
                emptyLabel.Font = Enum.Font.Gotham
                emptyLabel.TextXAlignment = Enum.TextXAlignment.Center
                emptyLabel.Parent = roomScroll
            end
        else
            healthLabel.Text = "💚 Joueur déconnecté"
            posLabel.Text = "📍 Position: N/A"
            micLabel.Text = "🎤 Voice Chat: N/A"
...
local function CreateMainGUI()
    ...
    title.BackgroundTransparency = 1
    title.Text = "🕵️ SPY MENU - THE CORNER"
    title.TextColor3 = Colors.Accent
    title.TextSize = 20
    title.Font = Enum.Font.GothamBold
    title.TextXAlignment = Enum.TextXAlignment.Left
    ...
    local roomsPage = Instance.new("Frame")
    roomsPage.Name = "RoomsPage"
    roomsPage.Size = UDim2.new(1, 0, 1, 0)
    roomsPage.BackgroundTransparency = 1
    roomsPage.Visible = false
    roomsPage.Parent = contentFrame
    
    local roomsScroll = Instance.new("ScrollingFrame")
    roomsScroll.Size = UDim2.new(1, -20, 1, -20)
    roomsScroll.Position = UDim2.new(0, 10, 0, 10)
    roomsScroll.BackgroundTransparency = 1
    roomsScroll.BorderSizePixel = 0
    roomsScroll.ScrollBarThickness = 8
    roomsScroll.ScrollBarImageColor3 = Colors.Accent
    roomsScroll.Parent = roomsPage
    
    local roomsLayout = Instance.new("UIListLayout")
    roomsLayout.Padding = UDim.new(0, 10)
    roomsLayout.SortOrder = Enum.SortOrder.LayoutOrder
    roomsLayout.Parent = roomsScroll
    
    local function RefreshRoomsPage()
        -- Nettoyer
        for _, child in pairs(roomsScroll:GetChildren()) do
            if child:IsA("Frame") then child:Destroy() end
        end
        
        -- Obtenir toutes les rooms actives
        local rooms = GetAllRooms()
        
        if #rooms == 0 then
            local emptyCard = Instance.new("Frame")
            emptyCard.Size = UDim2.new(1, -10, 0, 80)
            emptyCard.BackgroundColor3 = Colors.Secondary
            emptyCard.BorderSizePixel = 0
            emptyCard.Parent = roomsScroll
            ...
            msgLabel.BackgroundTransparency = 1
            msgLabel.Text = "🔒 Aucune private room active\n\nLes rooms privées se créent quand un joueur pose une porte et y entre"
            msgLabel.TextColor3 = Colors.TextSecondary
            msgLabel.TextSize = 13
            msgLabel.Font = Enum.Font.Gotham
            msgLabel.TextWrapped = true
            msgLabel.Parent = emptyCard
            
            return
        end
        
        -- Créer une carte par room
        for roomIndex, room in ipairs(rooms) do
            local roomCard = Instance.new("Frame")
            roomCard.Size = UDim2.new(1, -10, 0, 100 + (#room.Players * 50))
            roomCard.BackgroundColor3 = Colors.Secondary
            roomCard.BorderSizePixel = 0
            roomCard.LayoutOrder = roomIndex
            roomCard.Parent = roomsScroll
            
            local roomCorner = Instance.new("UICorner")
            roomCorner.CornerRadius = UDim.new(0, 10)
            roomCorner.Parent = roomCard
            ...
            titleLabel.Text = "🔒 Private Room #" .. roomIndex .. " - Owner: " .. room.Host.DisplayName
            titleLabel.TextColor3 = Colors.Accent
            titleLabel.TextSize = 16
            titleLabel.Font = Enum.Font.GothamBold
            titleLabel.TextXAlignment = Enum.TextXAlignment.Left
            titleLabel.Parent = roomCard
            ...
            countLabel.Text = "👥 " .. #room.Players .. " joueur(s)"
            countLabel.TextColor3 = Colors.TextSecondary
            countLabel.TextSize = 12
            countLabel.Font = Enum.Font.Gotham
            countLabel.TextXAlignment = Enum.TextXAlignment.Left
            countLabel.Parent = roomCard
            
            -- Bouton TP vers la room
            local btnTeleport = CreateButton("🚀 Téléporter", UDim2.new(0, 120, 0, 30), function()
                GotoPlayer(room.Host)
                Notify("Téléportation vers Room #" .. roomIndex, 2, Colors.Success)
            end)
            btnTeleport.Position = UDim2.new(1, -130, 0, 35)
            btnTeleport.Parent = roomCard
            
            -- Liste des joueurs
            local yOffset = 75
            for i, roomPlayer in ipairs(room.Players) do
                local playerFrame = Instance.new("Frame")
                playerFrame.Size = UDim2.new(1, -20, 0, 45)
                playerFrame.Position = UDim2.new(0, 10, 0, yOffset)
                playerFrame.BackgroundColor3 = Colors.Tertiary
                playerFrame.BorderSizePixel = 0
                playerFrame.Parent = roomCard
                ...
                nameLabel.BackgroundTransparency = 1
                nameLabel.Text = roomPlayer.DisplayName
                nameLabel.TextColor3 = Colors.Text
                ...
                local usernameLabel = Instance.new("TextLabel")
                usernameLabel.Size = UDim2.new(0.5, -10, 0, 16)
                usernameLabel.Position = UDim2.new(0, 10, 0, 25)
                usernameLabel.BackgroundTransparency = 1
                usernameLabel.Text = "@" .. roomPlayer.Name
                usernameLabel.TextColor3 = Colors.TextSecondary
                ...
                -- Boutons actions
                local btnGoto = CreateButton("🚀", UDim2.new(0, 35, 0, 35), function()
                    GotoPlayer(roomPlayer)
                end)
                btnGoto.Position = UDim2.new(1, -115, 0, 5)
                btnGoto.Parent = playerFrame
                
                local btnSpy = CreateButton("🧠", UDim2.new(0, 35, 0, 35), function()
                    OpenSpyWindow(roomPlayer)
                end)
                btnSpy.Position = UDim2.new(1, -75, 0, 5)
                btnSpy.Parent = playerFrame
                
                local btnWatch = CreateButton("👁️", UDim2.new(0, 35, 0, 35), function()
                    if SpyMenu.WatchedPlayers[roomPlayer.Name] then
                        SpyMenu.WatchedPlayers[roomPlayer.Name] = nil
                        Notify("Surveillance arrêtée pour " .. roomPlayer.DisplayName, 2, Colors.Success)
                    else
                        SpyMenu.WatchedPlayers[roomPlayer.Name] = {
                            UserId = roomPlayer.UserId,
                            LastSeen = os.time()
                        }
                        Notify("👁️ " .. roomPlayer.DisplayName .. " ajouté à la surveillance!", 2, Colors.Warning)
                    end
                end)
                btnWatch.Position = UDim2.new(1, -35, 0, 5)
                btnWatch.Parent = playerFrame
                
                yOffset = yOffset + 50
            end
        end
        
        roomsScroll.CanvasSize = UDim2.new(0, 0, 0, roomsLayout.AbsoluteContentSize.Y + 10)
    end
    
    -- Update automatique toutes les 3 secondes
    task.spawn(function()
        while task.wait(3) do
            if SpyMenu.CurrentTab == "Rooms" then
                RefreshRoomsPage()
            end
        end
    end)
...
print("╔═══════════════════════════════════════════════════════════╗")
print("║                                                           ║")
print("║        SPY MENU - THE CORNER EDITION - v1.0.0             ║")
...
print("  • Players: Liste avec filtres + Surveillance")
print("  • Rooms: Toutes les rooms actives avec TP")
print("  • SPY Direct: Proximité vocale + Détection de rooms")
print("  • K/D: Basé sur Robux donnés/reçus")
